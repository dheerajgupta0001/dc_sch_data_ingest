...
def getDayAheadUnitNameForState(stateName) -> pd.DataFrame():
    """_summary_
    ...
    try:
        ...
        dbCur = dbConn.cursor()
        # insert the raw data
        sql_fetch = "select id, day_ahead_dc_file_tag, plant_name from public.mapping_table where state in ('{0}')".format(stateName)
        dbCur.execute(sql_fetch)
        # unitData = dbCur.fetchall()
        unitDf = pd.read_sql(sql_fetch, dbConn)
        unitList = unitDf['day_ahead_dc_file_tag'].to_list()
        plantList = unitDf['plant_name'].to_list()
        plantId = unitDf['id'].to_list()
        unitDf = unitDf.dropna(subset=['day_ahead_dc_file_tag'])

    except Exception as err:
        print('Error while fetching day ahead dc unit name for {} from master table'.format(stateName))
        print(err)
