...
def getUnitNameForState(stateName) -> pd.DataFrame():
    ...
    try:
        ...
        dbCur = dbConn.cursor()
        # insert the raw data
        sql_fetch = "select id, intraday_dc_file_tag, plant_name from public.mapping_table where state in ('{0}')".format(stateName)
        dbCur.execute(sql_fetch)
        # unitData = dbCur.fetchall()
        unitDf = pd.read_sql(sql_fetch, dbConn)
        unitList = unitDf['intraday_dc_file_tag'].to_list()
